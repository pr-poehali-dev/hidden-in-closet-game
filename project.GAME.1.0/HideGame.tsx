...
export default function HideGame() {
  ...
  const startGame = useCallback(() => {
    ...
    s.shake = 0;
    s.gameState = "playing";
    setUiState({ hp: MAX_HP, gameState: "playing", inCloset: false, nearCloset: false, surviveTimer: 0, winTime: s.WIN_TIME, screaming: false });
  }, []);
    ...
    >
        ...
        <div style={{
          ...
          <div style={{ textAlign: "center", padding: "0 32px" }}>
            ...
            <button onClick={startGame} style={{
              background: "transparent", color: "#ff2200", border: "3px solid #cc0000",
              padding: "16px 40px", fontSize: "clamp(9px, 1.8vw, 13px)",
              ...
              onMouseLeave={e => { e.currentTarget.style.background = "transparent"; e.currentTarget.style.color = "#ff2200"; }}
            >НАЧАТЬ</button>
          </div>
        </div>
      )}
        ...
        <div style={{
          ...
          <div style={{ textAlign: "center", padding: "0 32px" }}>
            <div style={{
              ...
              marginBottom: "20px", animation: "flicker 0.2s infinite",
            }}>ТЫ УМЕР</div>
            <div style={{ color: "#551111", fontSize: "clamp(7px, 1.3vw, 10px)", marginBottom: "48px", letterSpacing: "2px" }}>
              ОН НАШЁЛ ТЕБЯ
            </div>
            <button onClick={startGame} style={{
              background: "transparent", color: "#cc3333", border: "2px solid #440000",
              padding: "14px 32px", fontSize: "clamp(8px, 1.5vw, 11px)",
              fontFamily: "'Press Start 2P', monospace", cursor: "pointer",
        ...
        <div style={{
          ...
          <div style={{ textAlign: "center", padding: "0 32px" }}>
            <div style={{
              ...
              textShadow: "0 0 20px #44ff66, 0 0 40px #44ff6644", marginBottom: "20px",
            }}>ТЫ ВЫЖИЛ</div>
            <div style={{ color: "#226633", fontSize: "clamp(7px, 1.3vw, 10px)", marginBottom: "48px", letterSpacing: "2px" }}>
              60 СЕКУНД ПОЗАДИ
            </div>
            <button onClick={startGame} style={{
              background: "transparent", color: "#44ff66", border: "2px solid #116622",
              padding: "14px 32px", fontSize: "clamp(8px, 1.5vw, 11px)",
              fontFamily: "'Press Start 2P', monospace", cursor: "pointer",
              letterSpacing: "2px", transition: "all 0.15s",
            }}
        ...
        {uiState.gameState === "playing" && (
          <>
            ...
            <div style={{
              ...
              fontSize: "clamp(5px, 1vw, 7px)", letterSpacing: "2px",
      ...
      {uiState.screaming && (
        <div style={{
          ...
          <svg viewBox="0 0 400 400" preserveAspectRatio="xMidYMid slice" style={{
            ...
            {[90,118,146,174,202,226,254,282].map((x, i) => (
              <polygon key={`ut${i}`} points={`${x},275 ${x+20},275 ${x+10},${i%2===0?315:305}`} fill={i%3===0?"#eeeecc":"#ddddbb"}/>
            ))}
            {[95,123,151,179,207,235,263].map((x, i) => (
              <polygon key={`lt${i}`} points={`${x},310 ${x+18},310 ${x+9},${285+i%2*8}`} fill={i%3===0?"#ddddaa":"#cccc99"}/>
            ))}
          ...
          <div style={{
            ...
            animation: "whiteFlash 0.12s steps(1) 3", pointerEvents: "none",
          }} />
          <div style={{
            position: "absolute", bottom: "5%", left: 0, right: 0, textAlign: "center",
            color: "#fff", fontSize: "clamp(14px, 4vw, 32px)",
            fontFamily: "'Press Start 2P', monospace",
            textShadow: "0 0 10px #fff, 0 0 30px #ff0000, 0 0 60px #ff0000",
            letterSpacing: "6px", animation: "textFlick 0.09s steps(1) infinite",
          }}>ОН НАШЁЛ ТЕБЯ</div>
        </div>
      )}

      <style>{`
        @keyframes flicker { 0%,100%{opacity:1} 33%{opacity:0.6} 66%{opacity:0.9} }
        @keyframes pulse { 0%,100%{opacity:1} 50%{opacity:0.4} }
        @keyframes screamFlash {
          0%{background:#000} 15%{background:#ff0000} 30%{background:#000}
          50%{background:#cc0000} 70%{background:#000} 85%{background:#880000} 100%{background:#000}
        }
        @keyframes bgPulse {
          0%{opacity:1;transform:scale(1)} 33%{opacity:0.7;transform:scale(1.05)}
          66%{opacity:1;transform:scale(0.98)} 100%{opacity:0.8;transform:scale(1.02)}
        }
        @keyframes faceShake {
        ...
        @keyframes whiteFlash { 0%{opacity:0.9} 33%{opacity:0} 66%{opacity:0.6} 100%{opacity:0} }
        @keyframes textFlick {
          0%{opacity:1;transform:scaleX(1)} 25%{opacity:0.3;transform:scaleX(1.04) translateY(-2px)}
          50%{opacity:1;transform:scaleX(0.98)} 75%{opacity:0.6;transform:scaleX(1.02) translateY(2px)}
